...
def ajustar_vecm(df_pair, k_ar_diff=2, coint_rank=1, deterministic='ci'):
    endog = df_pair.dropna()
    vecm = VECM(endog, k_ar_diff=k_ar_diff, coint_rank=coint_rank, deterministic=deterministic)
    fit = vecm.fit()
    return fit
...
if __name__ == "__main__":
    ...
    for _, row in pares.iterrows():
        ...
        if len(df_pair_test) < 30:
            print(f"Saltando par {A}-{B} por pocos datos en test ({len(df_pair_test)})")
            continue
        df_hist, trades_df, metrics = backtest_pair(df_pair_test)
        df_hist['Pair'] = f"{A}-{B}"
        resultados_all.append(df_hist)
        resumen_metrics.append((f"{A}-{B}", metrics))
        print(f"Metrics {A}-{B}:", metrics)

    # Concatenar y mostrar equity finales
    if resultados_all:
        df_all = pd.concat(resultados_all)
        last_vals = df_all.groupby('Pair')['pv'].last()
        last_vals.plot(kind='bar', title='Valor final por par (periodo test)')
        plt.show()

        plt.figure(figsize=(10,5))
        for dfh in resultados_all:
            plt.plot(dfh.index, dfh['pv'], alpha=0.6)
        plt.title('Equity curve por par (periodo test)')
        plt.show()
    else:
        print("No hay resultados de backtest (no se seleccionaron pares o datos insuficientes).")
